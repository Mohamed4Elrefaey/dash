...
import { Plus, Search, Users, Shield, Mail, Phone, Pencil, Trash2, X, CheckCircle, UserCircle } from "lucide-react"
    ...
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="text-start">
          ...
          <p className="text-sm text-muted-foreground">إدارة صلاحيات الموظفين والوصول للنظام</p>
        </div>
        <button
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3 text-start">
        <StatCard title="إجمالي الموظفين" value={staff.length.toString()} icon={Users} iconBgColor="bg-[#e8f5f1]" />
        <StatCard title="نشطون حالياً" value={staff.filter(s => s.status === "نشط").length.toString()} icon={CheckCircle} iconBgColor="bg-[#e8f5f1]" />
        <StatCard title="غير نشط" value={staff.filter(s => s.status === "غير نشط").length.toString()} icon={X} iconBgColor="bg-destructive/10" />
      </div>
      ...
      <div className="rounded-xl border border-border bg-card text-start">
        <div className="border-b border-border px-5 py-4">
          <h2 className="font-bold text-foreground">قائمة الموظفين ({filteredStaff.length})</h2>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/50">
                <th className="px-5 py-3 text-start font-semibold text-foreground">الموظف</th>
                <th className="px-5 py-3 text-start font-semibold text-foreground">الدور / الصلاحية</th>
                <th className="px-5 py-3 text-start font-semibold text-foreground">بيانات التواصل</th>
                <th className="px-5 py-3 text-start font-semibold text-foreground">الحالة</th>
                <th className="px-5 py-3 text-start font-semibold text-foreground">تاريخ الانضمام</th>
            ...
            <tbody>
              {filteredStaff.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-5 py-12 text-center text-muted-foreground">لا توجد نتائج مطابقة للبحث</td>
                </tr>
              ) : (
                filteredStaff.map((member) => (
                  <tr key={member.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-5 py-3">
                      <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${member.status === "نشط" ? "bg-[#e8f5f1] text-[#2d7a6b]" : "bg-destructive/10 text-destructive"}`}>
                        {member.status}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-muted-foreground">{member.joinDate}</td>
                    <td className="px-5 py-3">
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-card shadow-xl p-6 text-start">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xl font-bold text-foreground">إضافة موظف جديد</h2>
              <button onClick={() => setIsAddModalOpen(false)} className="rounded-lg p-1 text-muted-foreground hover:bg-muted"><X className="h-5 w-5" /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="mb-2 block text-sm font-semibold text-foreground">اسم الموظف</label>
                <input type="text" className={inputClass} placeholder="ادخل الاسم الكامل" />
              </div>
              <div>
                <label className="mb-2 block text-sm font-semibold text-foreground">البريد الإلكتروني</label>
                <input type="email" className={inputClass} placeholder="example@khatwa.com" />
              </div>
              <div>
            ...
            <div className="mt-8 flex justify-end gap-3">
              <button onClick={() => setIsAddModalOpen(false)} className="rounded-xl border border-border px-6 py-2.5 text-sm font-medium text-foreground hover:bg-muted">إلغاء</button>
              <button
                onClick={() => {
