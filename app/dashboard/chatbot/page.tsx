...
import { Bot, Send, User, Sparkles, MessageSquare, History, Search } from "lucide-react"
...
export default function ChatbotPage() {
  ...
  const getBotResponse = (input: string) => {
    ...
    if (text.includes("تطعيم")) return "يمكنك إدارة التطعيمات من قسم إدارة التطعيمات في القائمة الجانبية. هل تريد مني عرض جدول التطعيمات الإلزامي؟"
    if (text.includes("طفل")) return "لإضافة طفل جديد، انتقل إلى صفحة إدارة الأطفال واضغط على زر «إضافة طفل جديد». هل تحتاج لمساعدة في ملء البيانات؟"
    if (text.includes("طبيب") || text.includes("دكتور")) return "لدينا دليل شامل للأطباء والعيادات. يمكنك البحث بالتخصص أو الموقع الجغرافي."
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] flex-col gap-6 lg:flex-row">
      ...
      <div className="hidden w-80 shrink-0 flex-col gap-4 lg:flex">
        ...
        <div className="rounded-xl border border-border bg-primary/5 p-4 text-start">
          <div className="mb-2 flex items-center gap-2 font-bold text-primary">
            <Sparkles className="h-4 w-4" />
            نصيحة اليوم
          </div>
          <p className="text-xs leading-relaxed text-muted-foreground">
            تأكد من تحديث بيانات الأطفال بانتظام لضمان تلقي التنبيهات في المواعيد المحددة بدقة.
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        ...
        <div className="flex items-center gap-3 border-b border-border bg-card px-6 py-4">
          ...
          <div className="text-start">
            <h1 className="text-lg font-bold text-foreground">المساعد الذكي (Bot)</h1>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-muted/5">
          {messages.map((message) => (
            <div
              key={message.id}
              className={`flex ${message.sender === "user" ? "justify-start" : "justify-end"} gap-3`}
            >
              {message.sender === "user" && (
              ...
              <div className={`max-w-[80%] space-y-1`}>
                <div
                  className={`rounded-2xl px-4 py-3 text-sm shadow-sm text-start ${
                    message.sender === "user"
                      ? "bg-card border border-border text-foreground rounded-ts-none"
                      : "bg-primary text-primary-foreground rounded-te-none"
                  }`}
                >
                  {message.text}
                </div>
                <p className="text-[10px] text-muted-foreground px-1 text-start">
                  {message.time}
                </p>
              </div>
          ...
          {isTyping && (
            <div className="flex justify-end gap-3">
              <div className="rounded-2xl bg-primary px-4 py-3 text-primary-foreground rounded-te-none">
        ...
        <div className="border-t border-border p-4 bg-card">
          <div className="relative flex items-center gap-3">
            ...
            <button
              ...
              className="flex h-11 w-11 items-center justify-center rounded-xl bg-primary text-primary-foreground shadow-md hover:opacity-90 transition-opacity disabled:opacity-50"
            >
              <Send className="h-5 w-5 -rotate-90" />
            </button>
          </div>
          <p className="mt-2 text-center text-[10px] text-muted-foreground">
            قد يقوم المساعد الذكي بتقديم معلومات غير دقيقة أحياناً، يرجى مراجعة البيانات الرسمية.
          </p>
        </div>
      </div>
    </div>
